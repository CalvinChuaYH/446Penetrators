...
apt install -y python3 python3-pip python3-venv mysql-server git php #dependencies for app
echo "=== Securing MySQL ==="
...
mysql -u root -p'FlaskP@ss2025!' <<EOF
CREATE DATABASE flaskapp;
CREATE USER 'flaskuser'@'localhost' IDENTIFIED BY 'FlaskP@ss2025!';
...
echo "=== Creating flaskapp user and adding to adm group ==="
useradd -m -s /bin/bash flaskapp
echo "flaskpass123" | passwd --stdin flaskapp  # Simple pass; change for prod
usermod -aG adm flaskapp

echo "=== Setting up log file with vuln perms ==="
touch /var/log/flaskapp.log
chown root:adm /var/log/flaskapp.log
chmod 640 /var/log/flaskapp.log  # Read for adm group

echo "=== Creating project directory ==="
PROJECT_DIR="/home/flaskapp/446Penetrators"
mkdir -p "$PROJECT_DIR"
chown -R flaskapp:flaskapp "$PROJECT_DIR"
...
JWT_EXPIRY=60
EOF
chown flaskapp:flaskapp "$PROJECT_DIR/.env"
...
pip install flask flask-cors flask-sqlalchemy pymysql python-dotenv pyjwt werkzeug cryptography --trusted-host pypi.org --trusted-host files.pythonhosted.org
"
